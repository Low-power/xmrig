...
#   ifdef XMRIG_ALGO_ARGON2
    if (nextJob.algorithm().family() == Algorithm::ARGON2 && argon2::Impl::select(d_ptr->controller->config()->cpu().argon2Impl())) {
        XMRIG_LOG_INFO("%s use " WHITE_BOLD("argon2") " implementation " CSI "1;%dm" "%s",
                 tag,
...
void xmrig::CpuBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
        ...
        XMRIG_LOG_WARN(YELLOW_BOLD_S "CPU disabled, no suitable configuration for algo %s", job.algorithm().shortName());
...
void xmrig::CpuBackend::stop()
{
    ...
    XMRIG_LOG_INFO("%s" YELLOW(" stopped") BLACK_BOLD(" (%" PRIu64 " ms)"), tag, Chrono::steadyMSecs() - ts);
}
