...
void xmrig::Miner::printHashrate(bool details)
{
    ...
    XMRIG_LOG_INFO(WHITE_BOLD("speed") " 10s/60s/15m " CYAN_BOLD("%s") CYAN(" %s %s ") CYAN_BOLD("H/s") " max " CYAN_BOLD("%s H/s"),
...
void xmrig::Miner::setEnabled(bool enabled)
{
    ...
    if (enabled) {
        XMRIG_LOG_INFO(GREEN_BOLD("resumed"));
    }
    else {
        XMRIG_LOG_INFO(YELLOW_BOLD("paused") ", press " MAGENTA_BG_BOLD(" r ") " to resume");
    }
