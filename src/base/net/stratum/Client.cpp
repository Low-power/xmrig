...
void xmrig::Client::onResolved(const Dns &dns, int status)
{
    ...
    if (status < 0 && dns.isEmpty()) {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s] DNS error: \"%s\"", url(), uv_strerror(status));
        }
...
bool xmrig::Client::parseJob(const rapidjson::Value &params, int *code)
{
    ...
    if (!isQuiet()) {
        XMRIG_LOG_WARN("[%s] duplicate job received, reconnect", url());
    }
...
#   ifdef XMRIG_FEATURE_TLS
    ...
    XMRIG_LOG_DEBUG("[%s] TLS send     (%d bytes)", url(), static_cast<int>(buf.len));
...
bool xmrig::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
    if (!algorithm.isValid()) {
        if (!isQuiet()) {
            if (algo == nullptr) {
                XMRIG_LOG_ERR("[%s] unknown algorithm, make sure you set \"algo\" or \"coin\" option", url(), algo);
            }
            else {
                XMRIG_LOG_ERR("[%s] unsupported algorithm \"%s\" detected, reconnect", url(), algo);
            }
        }
    ...
    if (!ok && !isQuiet()) {
        XMRIG_LOG_ERR("[%s] incompatible/disabled algorithm \"%s\" detected, reconnect", url(), algorithm.shortName());
    }
...
int xmrig::Client::resolve(const String &host)
{
    ...
    if (!m_dns->resolve(host)) {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s:%u] getaddrinfo error: \"%s\"", host.data(), m_pool.port(), uv_strerror(m_dns->status()));
        }
...
void xmrig::Client::parse(char *line, size_t len)
{
    ...
    XMRIG_LOG_DEBUG("[%s] received (%d bytes): \"%.*s\"", url(), len, static_cast<int>(len), line);

    if (len < 32 || line[0] != '{') {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s] JSON decode failed", url());
        }
    ...
    if (doc.ParseInsitu(line).HasParseError()) {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s] JSON decode failed: \"%s\"", url(), rapidjson::GetParseError_En(doc.GetParseError()));
        }
...
void xmrig::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &error)
{
    if (error.IsObject()) {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s] error: \"%s\", code: %d", url(), error["message"].GetString(), error["code"].GetInt());
        }
        return;
    }
    ...
    XMRIG_LOG_WARN("[%s] unsupported method: \"%s\"", url(), method);
}
...
void xmrig::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (error.IsObject()) {
        ...
        if (!handleSubmitResponse(id, message) && !isQuiet()) {
            XMRIG_LOG_ERR("[%s] error: " RED_BOLD("\"%s\"") RED_S ", code: %d", url(), message, error["code"].GetInt());
        }
    ...
    if (id == 1) {
        ...
        if (!parseLogin(result, &code)) {
            if (!isQuiet()) {
                XMRIG_LOG_ERR("[%s] login error code: %d", url(), code);
            }
...
void xmrig::Client::read(ssize_t nread)
{
    ...
    if (nread < 0) {
        if (!isQuiet()) {
            XMRIG_LOG_ERR("[%s] read error: \"%s\"", url(), uv_strerror(static_cast<int>(nread)));
        }
...
#   ifdef XMRIG_FEATURE_TLS
    if (isTLS()) {
        XMRIG_LOG_DEBUG("[%s] TLS received (%d bytes)", url(), static_cast<int>(nread));
...
{
    XMRIG_LOG_DEBUG("[%s] state: \"%s\"", url(), states[state]);
